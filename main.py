...
@app.route('/remarques', methods=['GET'])
def get_remarques():
    token = request.headers.get('Authorization')
    if not token or not verify_token(token):
        return jsonify({"msg": "Accès non autorisé"}), 401
    ...
    remarques = Remarque.query.filter_by(user_id=user_id).filter(Remarque.date >= date_limite).all()
    return jsonify({ 'message': [remarque.to_dict() for remarque in remarques] })
...
def dernieres_remarques():
    ...
    # Récupérer les dernières remarques de chaque utilisateur
    last_remarks = {}
...
def add_chat_message():
    ...
    if content:
        ...
        message = Message(content=content, user_id=user.id, user_name=user.name)
...
def get_chat_messages():
    ...
    for payment_log in payment_logs:
        user = User.query.get(payment_log.user_id)
        paiement_info = {
            "user_name": user.name,
            "payment_type": payment_log.payment_type,
            "payment_date": payment_log.payment_date.strftime("%Y-%m-%d")
        }
